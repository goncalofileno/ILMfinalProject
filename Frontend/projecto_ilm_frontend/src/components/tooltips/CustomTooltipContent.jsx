...
const CustomTooltipContent = ({ task, fontSize, fontFamily }) => {
  ...
  const taskStatus = task.rawTask.status
    ? formatTaskStatus(task.rawTask.status)
    : formatStatusDropDown(task.rawTask.projectState);
  ...
  return (
    <div className={styles.tooltipDefaultContainer} style={style}>
      ...
      <p className={styles.tooltipDefaultContainerParagraph}>
        ...
        {getDurationInDays(task.start, task.end)} <Trans>day(s)</Trans>
      </p>
    </div>
  );
};
