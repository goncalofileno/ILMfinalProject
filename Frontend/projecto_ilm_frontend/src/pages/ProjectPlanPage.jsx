...
const ProjectPlanPage = () => {
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        setTaskDetails({}); // Reset task details to ensure state is up-to-date
      } catch (error) {
    ...
    const handleDateChange = async (task) => {
      // Procurar o membro inCharge diretamente em taskDetails, caso contrário, usar task.rawTask
      const inChargeUsername = taskDetails.inCharge || task.rawTask.inCharge;
  
      // Encontrar o membro inCharge no array de membros da task
      const inChargeMember = task.rawTask.membersOfTask.find(
      ...
      const updateTaskDto = {
        ...
        inChargeId: inChargeMember?.id || null, // Usar o id do inChargeMember encontrado
        systemProjectName: systemProjectName,
      };
